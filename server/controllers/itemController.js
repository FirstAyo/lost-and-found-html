...
function normalizeItemPayload(payload = {}, type = "lost", currentUser = null) {
  return {
    ...
    contactEmail: String(payload.contactEmail || currentUser?.email || "")
      .trim()
      .toLowerCase(),
...
function deriveLegacyLocation(payload) {
  if (payload.type === "found") {
    const foundLabel = buildCampusDisplayLabel(
      payload.foundLocation,
      payload.foundLocationOther,
    );
...
function validateItemPayload(payload) {
  const errors = [];
  const campusValues = CAMPUS_LOCATION_OPTIONS.map(
  ...
  if (payload.type === "found") {
    ...
    if (
      payload.foundLocation === "other" &&
      payload.foundLocationOther.length < 2
    ) {
    ...
    if (
      payload.pickupLocation === "other" &&
      payload.pickupLocationOther.length < 2
    ) {
  ...
  if (
    (payload.contactMethod === "email" || payload.contactMethod === "both") &&
    !payload.contactEmail.includes("@")
  ) {
    errors.push("Please enter a valid contact email.");
  }

  if (
    (payload.contactMethod === "phone" || payload.contactMethod === "both") &&
    payload.contactPhone.length < 7
  ) {
...
async function buildLocationPayload(payload) {
  ...
  const [foundLocationPreview, pickupLocationPreview] = await Promise.all([
    locationService.buildCampusLocationPreview(
      payload.foundLocation,
      payload.foundLocationOther,
    ),
    locationService.buildCampusLocationPreview(
      payload.pickupLocation,
      payload.pickupLocationOther,
    ),
  ]);
...
export const itemController = {
  ...
  renderCreateLost(req, res) {
    return res.render("pages/items/post-item", {
      ...
      formSubtitle:
        "Record where on the Langara campus you last saw the item and how people can reach you.",
  ...
  renderCreateFound(req, res) {
    return res.render("pages/items/post-item", {
      ...
      formSubtitle:
        "Show exactly where on the Langara campus the item was found and where the owner can verify and collect it.",
  ...
  async previewCampusLocationApi(req, res) {
    const locationValue = String(req.query.locationValue || "").trim();
    const otherText = String(req.query.otherText || "").trim();
    ...
    const preview = await locationService.previewCampusLocation(
      locationValue,
      otherText,
    );
